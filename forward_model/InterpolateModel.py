...
def InterpModel(teff, logg=4, metal=0, alpha=0, modelset='btsettl-cifist2011c', instrument='osiris', band='kbb'):
    ...
    def GetModel(temp, wave=False, **kwargs):
        ...
        if modelset.lower() == 'aces2013': 
                filename = 'ACES2013_t'+ str(int(temp)) + '_g' + '{0:.2f}'.format(float(logg)) + '_z' + '{0:.2f}'.format(float(metal)) + '_alpha' + '{0:.2f}'.format(float(alpha)) + '_kzz' + '{0:.2f}'.format(float(kzz)) + '_%s-%s-RAW.txt'%(instrument.upper(), band.upper()) 
